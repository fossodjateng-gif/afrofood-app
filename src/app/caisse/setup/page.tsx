...
export default function CaisseSetupPage() {
  ...
  async function loadConfig(role: StaffRole, userId: string) {
    ...
    try {
      ...
      const userProgress = readUserTapSetupProgress(userId);
      setHasServerConnection(true);
      setHasCompletedSetupBefore(
        userProgress.awarenessSeen && userProgress.termsAccepted && userProgress.educationSeen
      );
      setTapConfig({
        awarenessSeen: userProgress.awarenessSeen,
        termsViewed: userProgress.termsViewed,
        termsAccepted: userProgress.termsAccepted,
        educationSeen: userProgress.educationSeen,
        readinessStatus: "not_prepared",
      });
    } catch (e: unknown) {
  ...
  function saveUserProgress(next: Partial<UserTapSetupProgress>) {
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
          ...
          <section style={{ background: "white", border: `1px solid ${BRAND.border}`, borderRadius: 16, padding: 16, marginTop: 12 }}>
            ...
            <div style={{ marginTop: 14, display: "flex", gap: 10, flexWrap: "wrap" }}>
              <button
                className="af-btn"
                type="button"
                onClick={() => saveUserProgress({ awarenessSeen: true })}
                disabled={savingId === "config"}
          ...
          <section style={{ background: "white", border: `1px solid ${BRAND.border}`, borderRadius: 16, padding: 16, marginTop: 12 }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                saveUserProgress({ awarenessSeen: true, termsViewed: true, termsAccepted: true });
              }}
              disabled={savingId === "config"}
        ...
        {!loading && step === "education" ? (
          <section style={{ background: "white", border: `1px solid ${BRAND.border}`, borderRadius: 16, padding: 16, marginTop: 12 }}>
            ...
            <button
              ...
              type="button"
              onClick={() =>
                saveUserProgress({ awarenessSeen: true, termsViewed: true, termsAccepted: true, educationSeen: true })
              }
              disabled={savingId === "config"}
              style={{ marginTop: 14, padding: "12px 16px", borderRadius: 12, border: "none", background: "#111", color: "white", fontWeight: 900 }}
            >
        ...
        {!loading && step === "prepare" && !setupCompleted ? (
          <>
            <section style={{ background: "white", border: `1px solid ${BRAND.border}`, borderRadius: 16, padding: 16, marginTop: 12 }}>
            ...
            <section style={{ background: "white", border: `1px solid ${BRAND.border}`, borderRadius: 16, padding: 16, marginTop: 12 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 10,
                  border: "none",
                  background: connectionBlocked ? "#94a3b8" : "var(--af-accent-gradient)",
                  color: "white",
                  fontWeight: 900,
                  cursor: connectionBlocked ? "not-allowed" : "pointer",
                  opacity: connectionBlocked ? 0.8 : 1,
              ...
              <div style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <button
                  ...
                  onClick={() => {
                    setTermsChecked(false);
                    setHasCompletedSetupBefore(false);
                    saveUserProgress({ awarenessSeen: true, termsViewed: false, termsAccepted: false, educationSeen: false });
                  }}
